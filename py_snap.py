...
backup_file = '%s/%s%s.%s.tar' % (full_backup_dir,
                                  socket.gethostname(),
...
log.info('Backing up: %s to %s' % (args.logical_volume, full_backup_dir))
...
def backsnap(backup_dir):
    ...
    if args.compress:
        try:
            with tarfile.open('%s.gz' % backup_file, "w:gz") as tar:
                tar.add(relative_dir)

        except Exception, e:
            log.error(e)
            log.error('Could not create backup %s.gz (%s)') % (backup_file, e)
            sys.exit(2)
    else:
        try:
            with tarfile.open('%s.tar' % backup_file, "w") as tar:
                tar.add(relative_dir)
